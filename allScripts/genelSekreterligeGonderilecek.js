...
const genelSekreterligeGonderilecek = async () => {
  ...
  worksheet.eachRow((row, rowNumber) => {
    if (rowNumber > 1) {
      ...
      if (
        rowData["Kategori"] &&
        (rowData["Kategori"].includes("Bilgi İşlem Dairesi Başkanlığı") ||
          rowData["Kategori"].includes("EBYS ve E-imza İşlemleri"))
      ) {
        data.push(rowData);
      }
    }
  ...
  mergedCell.value = "Talebin Ulaştığı Birim: BİLGİ İŞLEM DAİRESİ BAŞKANLIĞI"; // İstenilen metin
  ...
  // Yeni dosyayı kaydet
  await newWorkbook.xlsx.writeFile("EYLÜL 2024 BİDB Talep Yönetim Sistemi Raporu.xlsx");
};

console.log("Genel Sekreterliğe Gönderilecek Rapor Oluşturuldu")

module.exports = {
    genelSekreterligeGonderilecek,
};
